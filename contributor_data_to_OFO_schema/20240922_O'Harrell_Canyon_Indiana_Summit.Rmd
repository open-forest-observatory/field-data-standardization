...
# merge plot elevations to tree data.frame

trees <- left_join (trees, plot_elevations_to_join, by = "Plot")

## now get tree elevations

# make into a spatial data.frame

trees <- st_as_sf(trees, coords = c("TreeLongitudeWGS84", "TreeLatitudeWGS84"), crs = 4236)

# use elevatr to find plot elevations

tree_elevations <- elevatr::get_elev_point(locations = trees, prj = st_crs(trees), src = "epqs") # "epqs" is the USGS Elevation Point Query Service

tree_elevations <- tree_elevations %>% rename (tree_elevation_meters = elevation)

trees <- full_join (tree_elevations, treecoordinates, by="ID")

## now add/subtract to get the tree heights. true tree height = plot center elevation + tree height above plot center elevation - tree elevation.

trees$height_m <- trees$plot_elevation_meters + trees$tree_height_above_plot_center_m - trees$tree_elevation_meters

#### there are a LOT of trees with negative heights, especially in plots OH1 and OH2. What if I get point elevation data from Amazon Web Service Terrain Tiles?
...
plot_elevations_aws <- elevatr::get_elev_point(locations = plots, prj = st_crs(plots), src = "aws", z = 14) # z is zoom, 14 is the maximum zoom

# make a smaller data.frame of plot numbers + elevations

plot_elevations_to_join <- plot_elevations_aws[,-c(1:2)]
...
plot_elevations_to_join <- plot_elevations_to_join %>% rename(plot_elevation_meters_aws = elevation)
...
tree_elevations_aws <- elevatr::get_elev_point(locations = trees, prj = st_crs(trees), src = "aws", z = 14, overwrite = TRUE)

trees <- tree_elevations_aws %>% rename (tree_elevation_meters_aws = elevation)

## now add/subtract to get the tree heights. true tree height = plot center elevation + tree height above plot center elevation - tree elevation.

trees$height_m_aws <- trees$plot_elevation_meters_aws + trees$tree_height_above_plot_center_m - trees$tree_elevation_meters_aws

#### the elevations and final heights are sooooooooooooooooooo different between datasets. using the aws dataset on the highest zoom level resulted in fewer negative heights, and less dramatically large negative heights. going to try using the OpenTopography API (gl1 elevation product) to generate a raster DEM and then extracting point elevations from that.
