...
DS_final <- DS_df %>%
  mutate(project_id = "0021",
    ...
    subplot_shape = "",
    subplot_area = "",
    includes_snags = FALSE, # change to TRUE if 2015 dead trees data is later
    ...
    canopy_cover = "",
    min_dbh = "4", # Data contributor believes that the presence of unmapped trees above the 4 cm DBH threshold should be minimal to nonexistent; if present, it is likely only several shrubby hardwoods like cherry and/or vine maple very close to the 4 cm threshold. Note that some trees below the threshold have been included in this dataset.
    min_dbh_live = "",
    max_dbh_of_primary_trees = "",
    min_ht = "", # height was only measured for a subset of treatment trees, there is no minimum height threshold
    min_dbh_ohvis = "",
    min_ht_ohvis = "",
    plot_lon = case_when(
...
combined <- left_join(data2015, contributed, by = "unique_id") %>%
  dplyr::select(unique_id, MortalityYear, HT_13, HT.m, DBH_13, DBH.cm, HLC_13, HLC.m, HTDAM_13, HeightDamage.code, Lean.code, TrtID, everything()) # you can see that most measurements should be slightly updated
...
# designating all treated trees as damage = mechanical, beside the soil disturbance trt
mechanical_codes <- c("B100-1S", "B20-1S", "B40-1F", "B40-1S", "B40-2S", "B60-1S", "B80-1S", "B80-2S", "B90-1S", "R25", "R50", "R25-B40-1S", "R50-B40-1S", "PRUNE") 
...
combined <- combined %>%
  ...
  ungroup() %>%
  dplyr::select(-dmg_ht, -dmg_trt, -dmg_lean)
...
# unfortunately, these weren't included in the geospatial data cleaning work done before the new dataset was discovered, so we can't include these because they currently lack coords
# remove these 522 dead trees for now:
DStrees <- DStrees %>% filter(!is.na(tree_lat))

# re-order trees by plot for cleaner export
DStrees <- DStrees[order(DStrees$plot_id),]
